@testset "CircularArrayBuffer" begin
    ...
    @testset "1D Int" begin
        ...
        @test_throws ArgumentError push!(b, [1, 2])
    ...
    @testset "2D Float64" begin
        ...
        @test size(b) == (2, 2, 0)
        # element must has the exact same length with the element of buffer
        @test_throws ArgumentError push!(b, [1.0, 2.0])
        ...
        @test size(b) == (2, 2, 3)
        for i = 1:3
            @test select_last_dim(b, i) == i * A
        end
        @test select_last_frame(b) == 3 * A
        ...
        @test capacity(b) == 3
        @test size(b) == (2, 2, 3)
        @test select_last_dim(b, 1) == 3 * A
        @test select_last_dim(b, 3) == 5 * A
